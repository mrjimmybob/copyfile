...
fn get_file_name(path: &str) -> Result<String, std::io::Error> {
    let file_name = Path::new(path).file_name().ok_or_else(|| std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid path"))?;
    let file_name = file_name.to_str().ok_or_else(|| std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid file name"))?;
...
fn print_file_size(file_path: &str) -> io::Result<()> {
    ...
    println!("        {} {}", Colour::Yellow.paint("File size:"), Colour::Cyan.paint(size));
    Ok(())
}
...
fn print_help() {
    ...
    println!("{}", Colour::Cyan.paint("Copy a file from source to destination."));
    println!("");
    println!("{}", "Usage:");
    println!("\t{} [{}] [{}] [{}] [{}] {} {}", Colour::Yellow.paint("copyfile"), Colour::Blue.paint("-v"), Colour::Blue.paint("-a"), Colour::Blue.paint("-f"), Colour::Blue.paint("-p"), Colour::Blue.paint("source"), Colour::Blue.paint("destination"));
    println!("\t{} {}", Colour::Yellow.paint("copyfile"), Colour::Blue.paint("-h"));
    println!("{}", Colour::Cyan.paint("Options:"));
    println!("\t{}\t\t{}", Colour::Cyan.paint("-v | --verbose"), Colour::Cyan.paint("Be verbose."));
    println!("\t{}\t\t{}", Colour::Cyan.paint("-p | --progress"), Colour::Cyan.paint("Show a progress bar."));
    println!("\t{}\t\t{}", Colour::Cyan.paint("-a | --ascii"), Colour::Cyan.paint("Use ascii characters instead of UTF-8 for the progress bar."));
    println!("\t{}\t\t{}", Colour::Cyan.paint("-f | --force"), Colour::Cyan.paint("Force overwrite if file exists at destination."));
    println!("\t{}\t\t{}", Colour::Cyan.paint("-h | --help"), Colour::Cyan.paint("Show this help."));
    println!("{} {}{}", Colour::Yellow.paint("Third"), Colour::Green.paint("3"), Colour::Yellow.paint("ye Software Inc. © 2024"));
...
fn print_failure(options: &Options, error: String) {
    if options.verbose {
        println!("");
        println!("{}{}", Colour::Red.paint("Error copying file: "), Colour::Red.paint(error));
    }
}

fn print_error(options: &Options, e: std::io::Error) {
    if options.verbose {
        let msg = format!("Error {} file: ", if options.move_file { "moving" } else { "copying" } );
        println!("");
        println!("{}{}", Colour::Red.paint(msg), Colour::Red.paint(e.to_string()));
    }
}

fn print_header(options: &Options) {
    if options.verbose {
        let msg = format!("{} file from:", if options.move_file { " Moving" } else { "Copying" } );
        println!("{} {}", Colour::Yellow.paint(msg), Colour::Cyan.paint(options.source.as_str()));
        println!("{} {}", Colour::Yellow.paint("               to:"), Colour::Cyan.paint(options.destination.as_str()));
        match print_file_size(options.source.as_str()) {
            Ok(_) => (),
            Err(e) => println!("{} {}", Colour::Red.paint("Error:"), Colour::Red.paint(e.to_string())),
        }
        if options.force {
            println!("{} {}", Colour::Yellow.paint("  Force overwrite:"), Colour::Cyan.paint("true"));
        }
        println!("");
    }
}
...
fn copy_file(options: &Options) -> std::io::Result<()> {
    ...
    let file_size = get_file_size(options.source.as_str()).unwrap();
    
    let mut col = 0;
    let mut acum = 0;

    let src_file = File::open(options.source.as_str())?;
    ...
    if options.verbose {
        ...
        println!("");
        println!("");
        println!("{}", Colour::Green.paint(format!("{} file succesful in {}", if options.move_file { "Move" } else { "Copy" }, time)));
    }
    ...
    if got_src && got_dst {
            ...
            if !file_exists(options.source.as_str()) {
                ...
                match get_file_name(options.source.as_str()) {
                    Ok(file_name) => {
                        options.destination = concatenate_path(options.destination.as_str(), file_name.as_str());
                        proceed = true
                    },
